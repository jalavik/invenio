...
def _sort_from_cache(name, from_data=False):
    def _sorter(item):
        try:
            ...
            if results:
                cached_result = msgpack.loads(results)
                if from_data:
                    # Get value from sort_data
                    cached_result.get("sort_data", {}).get(name)
                else:
                    return cached_result.get(name)
        except Exception:
...
def sort_bwolist(bwolist, sort_key):
    ...
    if sort_key == "newest":
        ...
        if sort_key.endswith("_desc"):
            ...
            reverse = True
            sort_key = sort_key[:-5]
        bwolist.sort(key=_sort_from_cache(sort_key, from_data=True), reverse=reverse)
    return bwolist
