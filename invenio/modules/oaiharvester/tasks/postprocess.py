...
def post_process_selected(post_process):
    ...
    def _post_process_selected(obj, eng):
        try:
            post_process_list = obj.extra_data["_repository"]["postprocess"]
        except KeyError:
...
def author_list(obj, eng):
    ...
    for xml_file in xml_files_list:
        ...
        if match:
            ...
            if 'a_stylesheet' not in obj.extra_data["_repository"]["arguments"]:
                a_stylesheet = "authorlist2marcxml.xsl"
            else:
                a_stylesheet = obj.extra_data["_repository"]["arguments"]["a_stylesheet"]
...
def upload_step(obj, eng):
    ...
    if "f" in obj.extra_data["_repository"]["postprocess"]:
        # We have a filter.
        file_uploads = [
...
filter_step.description = "Filtering script"
