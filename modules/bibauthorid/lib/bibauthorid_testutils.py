...
def person_in_aidpersonidpapers(name, recid):
    ...
    result = run_sql("select * from aidPERSONIDPAPERS where name = %s"
...
def _generate_canonical_name(name):
    '''
    The canonical name from aidPERSONIDDATA.
    '''
    parts_of_name = name.split()
    return ''.join([parts_of_name[0][0], '.', parts_of_name[1], '.1'])
...
def is_test_paper_claimed(recid, bibref_table):
    result = run_sql("select * from aidPERSONIDPAPERS where bibrec=%s and flag=2 and bibref_table='%s' limit 1", (recid, bibref_table))
    if result:
        return True
    else:
        return False
...
def get_new_marc_for_test(author_name=None, co_authors_names=None,
    ...
    build_test_marcxml_field(record, 245, 'Rabbit Test Paper')
    if author_name:
