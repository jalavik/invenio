...
from invenio.bibauthorid_backinterface import get_rec_to_signatures_mapping
from invenio.bibauthorid_backinterface import get_signatures_of_paper as _get_signatures_of_paper
from invenio.bibauthorid_backinterface import modify_signature as _modify_signature
from invenio.bibauthorid_backinterface import remove_signatures as _remove_signatures
...
def rabbit(bibrecs, check_invalid_papers=False, personids_to_update_extids=None, verbose=False):
    ...
    if bconfig.RABBIT_USE_CACHED_PID:
        ...
        REC_SIG_CACHE = get_rec_to_signatures_mapping()

        def get_signatures_of_papers_cache(rec):
            return REC_SIG_CACHE[int(rec)]
        ...
        def add_signature_using_names_cache(sig, name, pid):
            ...
            REC_SIG_CACHE[sig[2]].append( (pid, sig[0], sig[1], sig[2], name) )
        ...
        def _delete_sig_from_cache(sig):
            sigs = REC_SIG_CACHE[sig[0]]
            for s in list(sigs):
                if int(sig[0]) == int(s[1]) and int(sig[1]) == int(s[2]):
                    sigs.remove(s)

        def remove_signatures_cache(sigs):
            for sig in sigs:
                _delete_sig_from_cache(sig)
                _remove_signatures((sig,))

        def _update_sig_in_cache_and_get_pids(old_sig, new_sig, new_name):
            sigs = REC_SIG_CACHE[old_sig[2]]
            pids = list()
            for s in list(sigs):
                if int(old_sig[0]) == int(s[1]) and int(old_sig[1]) == int(s[2]):
                    pids.append(s[0])
                    sigs.remove(s)
                    sigs.append( (s[0], new_sig[0], new_sig[1], new_sig[2], new_name) )
            return pids

        def modify_signature_cache(old_ref, rec, new_ref, new_name):
            current_pids = _update_sig_in_cache_and_get_pids( (old_ref[0], old_ref[1], rec),
                                                              (new_ref[0], new_ref[1], rec),
                                                              new_name )
            pn_cache_entry = PID_NAMES_CACHE[get_name_by_bibref(old_ref)]

            for p in current_pids:
                pn_cache_entry.remove(p)
                try:
                    PID_NAMES_CACHE[new_name].add(p)
                except KeyError:
                    PID_NAMES_CACHE[name] = set([p])

            _modify_signature(old_ref, rec, new_ref, new_name)
        ...
        get_signatures_of_paper = get_signatures_of_papers_cache
        remove_signatures = remove_signatures_cache
        modify_signature = modify_signature_cache
    else:
        ...
        get_signatures_of_paper = _get_signatures_of_paper
        remove_signatures = _remove_signatures
        modify_signature = _modify_signature
    ...
    if (bconfig.RABBIT_USE_CACHED_GET_GROUPED_RECORDS and
        ...
        populate_partial_marc_caches(bibrecs)
    ...
    for idx, rec in enumerate(bibrecs):
        ...
        new_signatures_names = dict((new, get_name_by_bibref(new)) for new in new_signatures)
